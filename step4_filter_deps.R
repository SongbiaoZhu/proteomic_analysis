...
rel.var_bar <-
  rel.var_bar +  geom_line(data = rel.var.table,
                           aes(
                             x = Var1,
                             y = (Cumulative) * rel.var.table$Freq[1],
                             group = 1
                           ),color="red",size=1,
                           inherit.aes = FALSE) +
  #specify secondary axis
  #apply inverse of above transformation to correctly scale secondary axis (/1000)
  scale_y_continuous(sec.axis = sec_axis( ~ . /  rel.var.table$Freq[1], name = "Cumulative Coverage")) +
