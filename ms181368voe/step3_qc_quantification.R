...
ratio.na <- is.na(dat1_symbol[, c("ratio129.126", "ratio130.127", "ratio131.128")])
...
print(paste0(
  dim(dat3_omna)[1],
  " proteins with ratios have more than 2 unique peptides"
))

print(paste0(
  dim(dat2_uni2)[1] - dim(dat3_omna)[1],
  " Proteins with more than 2 unique peptides but don't have ratio"
))
if(F){
  dat2_uni2[which(is.na(dat2_uni2$ratio129.126)), ][, c(
    "Accession",
    "Description",
    "Unique.Peptides",
    "ratio129.126",
...
reprodu_1 <-
  ggplotRegression(lm(ratio129.126 ~ ratio130.127, data = dat2_uni2[, 3:5])) +
  labs(x = "Replicate 1", y = "Replicate 2")
ggsave(
  "out_folder/reprodu_1.jpeg",
  dpi = 600,
  width = 8.5,
  height = 8.5
)
...
library(GGally)
reprodu_pairs <- ggpairs(
  log2(na.omit(dat2_uni2[, 3:5])),
...
reprodu_pairs_internal <- ggpairs(
  log2(na.omit(dat2_uni2[, 3:5])),
  upper = list(continuous = wrap("cor", size = 8)),
  lower = list(continuous = "smooth"),
  axisLabels = "internal"
...
# before PCA analysis
result.pca <-
  PCA(dat_pca[, -ncol(dat_pca)], graph = FALSE)#现在dat_pca最后一列是group_list，需要重新赋值给一个dat.pca,这个矩阵是不含有分组信息的
fviz_pca_ind(
  result.pca,
  geom.ind =  "point",
  # show points only (nbut not "text")
  col.ind = dat_pca$group_list,
  # color by groups
...
library(pheatmap)
pheatmap(dat3_omna[, paste0("scaledAbun", 126:131)][cg, ],
         show_colnames = F,
         show_rownames = F)
...
pheatmap(
  ...
  cellwidth = 60,
  annotation_col = ac,
  filename = 'out_folder/heatmap_top1000_sd.jpeg'
)
