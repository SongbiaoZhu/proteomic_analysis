...
for i in protid:
    if not '_{}'.format(orgFull) in i:
        print(i)
        print(protid.index(i))
        protid[protid.index(i)] = i.replace('_HOMNE', '_{}'.format(orgFull))
# So it's ok to extract the protein name from between '_HUMAN' and ' OS='
